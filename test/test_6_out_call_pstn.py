# Тест прововеряет, что звонок по пстн из аккаунта кита с ру номера( из софтфона) на софтфон платформы выполняется успешно
...
def test_call_out_pstn(page: Page, context: BrowserContext):
